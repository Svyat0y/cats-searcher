...
const SearchPanelButtons: FC = () => {
	...
	return (
		<div className={ s.icons }>
			<Link to={ `voting/likes?page=${ likePage + 1 }` } className={ `${ s.iconWrapper } ${ locLikes ? s.iconActive : '' }` }>
			...
			<Link to={ `voting/favourites?page=${ favPage + 1 }` } className={ `${ s.iconWrapper } ${ locFav ? s.iconActive : '' }` }>
			...
			<Link to={ 'voting/dislikes' } className={ `${ s.iconWrapper } ${ locDislikes ? s.iconActive : '' }` }>
				<svg width='30' height='30' viewBox='0 0 30 30' fill='none' xmlns='http://www.w3.org/2000/svg'>
					<path
