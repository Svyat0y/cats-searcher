...
const SearchPanelButtons: React.FC = () => {
	...
	return (
		<div className={ s.search__icons }>
			<Link to={ `voting/likes?page=${ likePage }` } className={ `${ s.icon_wr } ${ locLikes && s.iconActive }` }>
			...
			<Link to={ `voting/favourites?page=${ likePage }` } className={ `${ s.icon_wr } ${ locFav && s.iconActive }` }>
			...
			<Link to={ `voting/dislikes?page=${ likePage }` } className={ `${ s.icon_wr } ${ locDislikes && s.iconActive }` }>
				<svg width='30' height='30' viewBox='0 0 30 30' fill='none' xmlns='http://www.w3.org/2000/svg'>
					<path
